    ...
    <Container sx={{ py: { xs: 3, md: 6 } }} ref={rootRef}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1.1fr 0.9fr' }, alignItems: 'center', gap: { xs: 2, md: 4 }, mb: { xs: 4, md: 6 } }}>
        <Box className="fade-up hero-text" sx={{ pr: { md: 4 } }}>
          ...
          <Button variant="contained" size="large" onClick={() => navigate(curr.slug)} sx={{ fontWeight: 800, backgroundColor: 'var(--color-buy)', border: '1px solid #ff2f60', '&:hover': { backgroundColor: 'var(--color-buy-hover)' } }}>Explore {labelsBySlug[curr.id] || 'Now'}</Button>
